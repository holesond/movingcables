...
class RandomSamples(mx.operator.CustomOp):
    def forward(self, is_train, req, in_data, out_data, aux):
        ...
        self.idx = mx.nd.zeros((in_data[1].shape[0],in_data[1].shape[1]),
            dtype="int32")
        for i in range(in_data[1].shape[0]):
            s = mx.nd.random.shuffle(s)
            self.idx[i,:] = mx.nd.slice(s,0,in_data[1].shape[1])
...
@mx.operator.register("random_samples")
class RandomSamplesProp(mx.operator.CustomOpProp):
    def __init__(self):
        super(RandomSamplesProp, self).__init__()
